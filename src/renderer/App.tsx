...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-brand-bg text-brand-text font-sans selection:bg-brand-primary/30">
      <TopBar onShowOptions={() => setShowOptions(true)} />
          ...
          >
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              <div className="absolute -top-12 right-0">
                 <button 
                  onClick={() => setShowOptions(false)}
                  className="px-3 py-1 bg-brand-surface border border-brand-border rounded-lg text-[10px] font-bold uppercase tracking-widest text-brand-text/60 hover:text-brand-primary transition-colors focus:ring-2 focus:ring-brand-primary/50 outline-none"
                >
                  Close ESC
                </button>
              </div>
